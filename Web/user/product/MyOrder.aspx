<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyOrder.aspx.cs" Inherits="web.user.product.MyOrder" %>
...
<input name="textfield4" type="text" id="textEnd"  class="input_select" runat="server" tip="输入购买日期" onClick="WdatePicker()"/>
        ...
        <asp:LinkButton ID="LinkButton1" runat="server" CommandArgument='<%# Eval("OrderCode") %>'
         ...
         <asp:LinkButton ID="LinkButton3" runat="server"  Visible='<%#Eval("IsSend").ToString()=="0"?true:false %>'
        Enabled=false>未发货</asp:LinkButton>                        
        <asp:LinkButton ID="LinkButton2" runat="server" CommandArgument='<%# Eval("OrderCode") %>' Visible='<%#Eval("IsSend").ToString()=="1"?true:false %>'
        CommandName="take">确认收货</asp:LinkButton>
        </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
